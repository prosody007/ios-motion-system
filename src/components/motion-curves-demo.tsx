...
function PreviewTrack({ token, large = false }: { token: MotionCurveToken; large?: boolean }) {
  ...
  return <div className="min-w-0"><div className="flex min-w-0 items-center gap-4"><div ref={trackRef} className={`relative isolate h-10 min-w-0 flex-1 overflow-hidden rounded-full bg-[#e9edf5] ${large ? "h-16" : ""}`}><div className="pointer-events-none absolute inset-x-7 top-1/2 h-px -translate-y-1/2 bg-[#cbd2df]" style={large ? { left: inset, right: inset } : undefined} /><div className={`pointer-events-none absolute top-1/2 rounded-full ${size} ${isSpring ? "bg-[#111827] shadow-[0_12px_22px_rgba(17,24,39,.23)]" : "bg-[#2878f0] shadow-[0_10px_20px_rgba(40,120,240,.3)]"}`} style={{ left: position, transform: "translate(-50%, -50%)", ...(!isSpring ? { transition: `left ${token.durationMs}ms var(${token.cssVar})` } : {}) }} /></div><PlayButton label={isSpring ? "Run" : "Play"} ariaLabel={isSpring ? "Run spring preview" : "Play once"} onClick={() => isSpring ? runSpring() : setRunId((value) => value + 1)} /></div><div className="mt-3 min-w-0"><code className="block truncate font-mono text-[10px] text-[#8791a3]">{isSpring ? `stiff ${token.spring?.stiffness} · damp ${token.spring?.damping} · mass ${token.spring?.mass}` : token.cssVar}</code></div></div>;
}
...
function Explorer({ activeKind, setActiveKind, selectedId, setSelectedId }: { activeKind: "timing" | "spring"; setActiveKind: (value: "timing" | "spring") => void; selectedId: string | null; setSelectedId: (value: string | null) => void }) {
  const tokens = activeKind === "timing" ? timingTokens : springTokens;
  return <Panel className="overflow-hidden"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-[#e7ebf1] px-6 py-6 sm:px-7"><div><div className="font-mono text-[10px] text-[#2878f0]">01 / REFERENCE</div><h2 className="mt-1 text-[21px] font-semibold tracking-[-.04em] text-[#111827]">Curve explorer</h2><p className="mt-1 text-[11px] text-[#8791a3]">选择一个 token 查看它的实际手感与参数。</p></div><div className="flex gap-1.5"><Pill active={activeKind === "timing"} onClick={() => { setActiveKind("timing"); setSelectedId(null); }}>Timing · {timingTokens.length}</Pill><Pill active={activeKind === "spring"} onClick={() => { setActiveKind("spring"); setSelectedId(null); }}>Spring · {springTokens.length}</Pill></div></div><div>{tokens.map((token) => <TokenRow key={token.id} token={token} selected={token.id === selectedId} onSelect={() => setSelectedId(token.id)} />)}</div></Panel>;
}

function Inspector({ token }: { token: MotionCurveToken | null }) {
  if (!token) return <Panel className="overflow-hidden"><div className="border-b border-[#e7ebf1] px-6 py-6 sm:px-7"><div className="font-mono text-[10px] text-[#2878f0]">02 / INSPECTOR</div><h2 className="mt-2 text-[24px] font-semibold tracking-[-.05em] text-[#111827]">Select a curve</h2><p className="mt-2 text-[12px] leading-5 text-[#667085]">从左侧列表选择一个案例，查看它的预览、参数和使用方式。</p></div><div className="flex min-h-[228px] items-center justify-center bg-[#f5f7fa] px-6 py-7 text-center sm:px-7"><div><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-[20px] text-[#98a2b3] shadow-sm">＋</div><p className="mt-3 text-[11px] text-[#8791a3]">点击任意曲线开始检查</p></div></div></Panel>;
...
export function MotionCurvesDemo() {
  ...
  const selectedToken = selectedId ? motionCurveTokens.find((token) => token.id === selectedId) ?? null : null;
  return <div className="mx-auto w-full max-w-[1320px] pb-24"><header className="mb-8"><h1 className="text-[clamp(2.2rem,4vw,3.6rem)] font-semibold leading-none tracking-[-.065em] text-[#111827]">A better sense of motion.</h1><p className="mt-3 max-w-[620px] text-[13px] leading-6 text-[#667085]">用一致的曲线和物理参数，让每个界面状态变化都更自然、更可控。</p></header><div className="mb-5"><RulesCard /></div><div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_420px]"><Explorer activeKind={activeKind} setActiveKind={setActiveKind} selectedId={selectedId} setSelectedId={setSelectedId} /><Inspector token={selectedToken} /></div><div className="mt-5 grid items-start xl:grid-cols-[minmax(0,1fr)_420px]"><div className="hidden xl:block" /><BezierUsage /></div><div className="mt-5"><SpringSelection /></div><div className="mt-5"><IosSpringExamples /></div></div>;
}
