...
function Inspector({ token }: { token: MotionCurveToken }) {
  return <Panel className="overflow-hidden"><div className="border-b border-[#e7ebf1] px-6 py-6 sm:px-7"><div className="flex items-center justify-between"><div className="font-mono text-[10px] text-[#2878f0]">02 / INSPECTOR</div><span className="rounded-full bg-[#111827] px-2.5 py-1 text-[10px] text-white">{token.kind === "spring" ? "SPRING" : "CSS"}</span></div><h2 className="mt-2 text-[24px] font-semibold tracking-[-.05em] text-[#111827]">{token.name}</h2><p className="mt-2 text-[12px] leading-5 text-[#667085]">{token.note}</p></div><div className="bg-[#f5f7fa] px-6 py-7 sm:px-7"><PreviewTrack key={token.id} token={token} large /></div><div className="grid gap-px bg-[#e7ebf1] sm:grid-cols-2"><div className="bg-white px-6 py-5 sm:px-7"><div className="text-[10px] uppercase tracking-[.12em] text-[#98a2b3]">Recommended use</div><div className="mt-2 text-[13px] font-semibold text-[#111827]">{token.use}</div></div><div className="bg-white px-6 py-5 sm:px-7"><div className="text-[10px] uppercase tracking-[.12em] text-[#98a2b3]">Duration</div><div className="mt-2 font-mono text-[13px] text-[#475467]">{token.durationMs}ms</div></div></div>{token.spring ? <div className="grid grid-cols-3 gap-2 px-6 py-5 sm:px-7">{[["stiffness", token.spring.stiffness], ["damping", token.spring.damping], ["mass", token.spring.mass]].map(([label, value]) => <div key={label} className="rounded-2xl bg-[#f3f5f8] px-3 py-3"><div className="text-[9px] uppercase tracking-[.1em] text-[#98a2b3]">{label}</div><div className="mt-1 font-mono text-[13px] text-[#111827]">{value}</div></div>)}</div> : <div className="px-6 py-5 sm:px-7"><code className="block rounded-xl bg-[#f3f5f8] px-3 py-3 font-mono text-[11px] text-[#667085]">{token.easing}</code></div>}</Panel>;
}
