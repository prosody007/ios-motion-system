...
function TokenRow({ token }: { token: MotionCurveToken }) {
  const meta = token.kind === "spring" && token.spring ? `stiffness ${token.spring.stiffness} · damping ${token.spring.damping} · mass ${token.spring.mass}` : token.cssVar;
  return <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-x-4 gap-y-3 border-b border-[#edf0f4] px-6 py-4 last:border-0 sm:grid-cols-[300px_minmax(160px,1fr)_80px] sm:items-center sm:gap-4 sm:px-7"><div className="col-start-1 row-start-1 min-w-0 sm:col-auto sm:row-auto"><div className="truncate text-[13px] font-semibold text-[#111827]">{token.name.replace("Spring ", "")}</div><div className="mt-1 truncate font-mono text-[10px] text-[#98a2b3]">{meta}</div></div><div className="col-span-2 row-start-2 min-w-0 sm:col-auto sm:row-auto"><PreviewTrack token={token} showMeta={false} /></div><div className="col-start-2 row-start-1 whitespace-nowrap text-right sm:col-auto sm:row-auto"><div className="font-mono text-[11px] text-[#475467]">{token.durationMs}ms</div><div className="mt-1 text-[9px] uppercase tracking-[.1em] text-[#98a2b3]">duration</div></div></div>;
}
